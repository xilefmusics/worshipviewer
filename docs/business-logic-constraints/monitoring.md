...
- **BLC-MON-005:** **`GET /api/v1/monitoring/metrics`** exposes one daily metrics entry per UTC calendar day touched by inclusive RFC 3339 `start` and `end` timestamps, and also accepts plain `YYYY-MM-DD` dates for both bounds; access is **admin-only** (same pattern as **BLC-MON-004** for audit logs). Query parameters and response shape are defined in OpenAPI.
