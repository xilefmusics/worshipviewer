...
During an audited support session, **BLC-IMP-011** applies: session lists and
revocation are scoped to the effective subject, while current-session and
current-session-metrics endpoints do not expose the actor's primary session.
