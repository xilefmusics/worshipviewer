...
- **BLC-SETL-019:** Each setlist **song** item MAY include **`language`** as a song language tag, an optional custom **`flow`**, and an optional **`capo_shape`** playing shape. Missing, **`null`**, or stale **`language`** tags inherit the song’s default language for player/export rendering. Missing or **`null`** **`flow`** means default song flow; a non-`null` flow is setlist-only and is threaded into Book rendering. A saved **`capo_shape`** is used as the standalone Sheet player baseline, while local playback changes do not mutate the setlist; Room playback ignores capo state.
...
- **BLC-SETL-021:** Canonical ordered content is **`items`**: tagged **`type: "song"`** entries (id, optional number/key/capo_shape/tempo/language/flow) and **`type: "media"`** entries (Media id only). Duplicate song or Media ids are independent slots by position and MUST NOT be deduplicated.
- **BLC-SETL-022:** Deleted, Failed, Processing, unreadable, or cross-team-revoked Media remains a stale **`items`** link on read and write. **`GET …/player?view=book`** (default), **`GET …/songs`**, exports, offline cache, and Room snapshots omit Media and rebuild contiguous song indexes. **`GET …/player?view=av`** includes each Ready Media the caller may currently read as one tagged immutable snapshot; stale Media is omitted without leaving gaps. Readable setlist access does not grant Media access.
