...
- **BLC-SETL-019:** Each setlist **`songs`** entry MAY include **`language`** as a song language tag and an optional custom **`flow`**. Missing, **`null`**, or stale **`language`** tags inherit the song’s default language for player/export rendering. Missing or **`null`** **`flow`** means default song flow; a non-`null` flow is setlist-only and is threaded into Book rendering, text-only (hide chords) Book rendering, setlist PDF/text export, and AV slides/navigation. WHEN a saved **`flow`** is invalid against the current song (for example a removed section) THEN all player and export surfaces fall back to the default flow for that slot without mutating persisted data. The setlist editor surfaces stale/invalid flow state and offers reset to default; it does not auto-repair saved flows on load.
