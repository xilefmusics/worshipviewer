...
The legacy app is a single-author, mostly-imperative wasm bundle (`frontend/src/{api,components,pages,app.rs,route.rs,main.rs}`). The new app is a full monorepo workspace (`frontend/{app,packages,crates}`) with a documented multi-epic roadmap in `frontend/docs/`.
...
New routes (from `frontend/app/src/routes/`):

| New route | Replaces legacy | Notes |
|---|---|---|
| `/login`, `/logout` | `/login`, `/logout` | Same purpose; new login has OAuth + OTP **tabs** plus `return_to` allowlist (`frontend/app/src/routes/login.tsx`) |
...
New `frontend/app/src/components/setlists/SetlistEditorScreen.tsx` features:
...
`frontend/app/src/components/player/PlayerBook.tsx` (≈ 820 lines) + `PlayerRoute.tsx` + slides (`BlobSlide`, `ChordsSlide`, `ChordsThreeColumnSlide`) + `PlayerBookSpread` + `PlayerTocSidebar`. New capabilities the legacy never had:
...
`frontend/app/src/components/player/av/PlayerAv.tsx` is the **new presenter**. It uses the same player resolution pipeline as Book mode (single source of truth) and renders:
...
- Background presets that **explicitly preserve the legacy semantics**: `0 = Black, 1 = Red, 2 = Ray` — see `frontend/app/src/lib/player/av-preferences.ts`:
...
|---|---|---|
| Player ↔ Presenter mode switcher in the player **top bar** (`TopbarSelect`) | **Dropped** | Mode is decided **before** entry (default in Settings; "Play in Normal / AV mode" entries in the hub context menu). E8.1 grill explicitly decided **no in-player mode toggle**: see `frontend/docs/plan.md` decision log entry `2026-05-23 E8.1`. |
...
The **transport** for the projection window also changed: legacy used `localStorage` + `StorageEvent`, new uses `BroadcastChannel` (`createAvProjectionSync` in `frontend/app/src/lib/player/av-projection-sync.ts`). Functionally similar, but the channel name carries a session id, which is more correct for multi-tab setups.
...
All four selected items are AV-mode behavior. They share two files (`frontend/app/src/lib/player/av-keyboard.ts` and `frontend/app/src/components/player/av/PlayerAv.tsx`) and one preferences file (`frontend/app/src/lib/player/av-preferences.ts`), so they should ship as one cohesive change.
...
Recommended commit order (each commit independently passing `pnpm -C frontend test`):
...
5. Add i18n strings, update `legacy-vs-frontend2-analysis.md` and `frontend/docs/plan.md` decision log.

### 11.2 Detailed work breakdown

#### Step 1 — Pre-Chorus and Verse 5–9 keyboard jumps (covers f12 + f13)

**File**: `frontend/app/src/lib/player/av-keyboard.ts`
...
**Tests**: `frontend/app/src/lib/player/av-keyboard.test.ts`
...
**File**: `frontend/app/src/lib/player/av-preferences.ts`
...
**File**: `frontend/app/src/components/player/av/AvSlideView.tsx`
...
**File**: `frontend/app/src/components/player/av/AvOutputPage.tsx`

- Update the default `viewPayload` to `screenState: 'live'`.
- Pass `screenState` through to `AvSlideView`.

**File**: `frontend/app/src/components/player/av/AvSectionShortcuts.tsx`
...
**File**: `frontend/app/src/components/player/av/PlayerAv.tsx`
...
**File**: `frontend/app/src/i18n/en.json` (and `de.json`)
...
**File**: `frontend/app/src/i18n/de.json` — mirror translations.

#### Step 6 — Docs

- Add a one-line entry to `frontend/docs/plan.md` decision log:
...
- Update `frontend/docs/legacy-vs-frontend2-analysis.md`:
...
1. `pnpm -C frontend/app test --run` (Vitest suite, ensure ≥242 green).
2. `pnpm -C frontend/app typecheck`.
3. `pnpm -C frontend/app lint`.
4. `pnpm -C frontend/app build`.
...
- `AvScreenState` replaces every `blackout: boolean` reference in the AV module (verified by `grep blackout frontend/app/src/{lib,components}/player/av`).
...
- `frontend/docs/plan.md` includes the new decision-log entry.
