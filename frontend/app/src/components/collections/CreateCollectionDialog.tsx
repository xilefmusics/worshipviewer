...
export function CreateCollectionDialog({ open, onOpenChange, onCreated }: CreateCollectionDialogProps) {
  ...
  const teamsQ = useInfiniteQuery({
    queryKey: [...teamsListRootKey, 'collectionCreate', ''] as const,
    initialPageParam: 0,
    enabled: open,
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      if (!trimmed) throw new Error(t('collections.create.titleRequired'))
      const body: { title: string; songs: []; cover: string; owner?: string } = {
        title: trimmed,
        cover: '',
        songs: [],
      }
                ...
                >
                  ...
                  <div className="flex flex-col gap-2 text-center sm:text-left">
                    <Dialog.Title className="text-lg font-semibold leading-none">
                      {t('collections.create.dialogTitle')}
                    </Dialog.Title>
                    <p className="text-sm text-[var(--color-muted-foreground)]">
                      {t('collections.create.dialogDescription')}
                    </p>
                  </div>
                  <div className="grid gap-2">
                    <label htmlFor="collection-create-title" className="text-sm font-medium">
                    ...
                    {showOwnerPicker ? (
                      <div className="grid gap-1.5 text-sm font-medium">
                        <label htmlFor="collection-create-owner">{t('collections.create.teamLabel')}</label>
                        <Select value={ownerId} onValueChange={(v) => setOwnerPick(v)}>
                          <SelectTrigger id="collection-create-owner" className="font-normal">
                            <SelectValue />
                          </SelectTrigger>
                          <SelectContent>
                  ...
                  <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
                    ...
                    >
                      {mutation.isPending ? t('common.load') : t('collections.create.submit')}
                    </Button>
                  </div>
                </motion.div>
              </Dialog.Content>
