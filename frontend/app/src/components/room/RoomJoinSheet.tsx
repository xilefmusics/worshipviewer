...
import {
  type RoomJoinModeChoice,
  roomJoinModeChoiceToWire,
} from '@/lib/room-join-mode'
import { cn } from '@/lib/utils'

const MODE_CHOICES: RoomJoinModeChoice[] = ['chords', 'text', 'av', 'slide']
...
type RoomJoinSheetProps = {
  ...
  onJoin: (choice: RoomJoinModeChoice) => void | Promise<void>
  displayName?: string
  onDisplayNameChange?: (name: string) => void
  pending?: boolean
}

export function RoomJoinSheet({
  sheetId,
  title,
  avOccupied,
  open,
  ...
  const [selectedMode, setSelectedMode] = useState<RoomJoinModeChoice>('chords')
  const [dragOffset, setDragOffset] = useState(0)
  const [isDragging, setIsDragging] = useState(false)
  const pointerStartY = useRef<number | null>(null)
  const descriptionId = `room-mode-description-${sheetId}`
  ...
  const joinSelectedMode = () => {
    const { hideChords } = roomJoinModeChoiceToWire(selectedMode)
                ...
                >
                  ...
                  {requiresDisplayName ? (
                    <label className="grid gap-2">
                      <span className="text-sm">{t('rooms.displayName')}</span>
                      <Input
                        value={displayName ?? ''}
                        maxLength={80}
                  ...
                  <div
                    role="tablist"
                    aria-label={t('rooms.modeChooserLabel')}
                    ...
                    {MODE_CHOICES.map((mode) => {
                      ...
                      return (
                        <button
                          ...
                          disabled={disabled}
                          title={disabled ? t('rooms.avOccupied') : undefined}
                  ...
                  >
                    {t(`rooms.modeDescription.${selectedMode}`)}
                  </p>
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <Button
                      ...
                      title={selectedMode === 'av' && avOccupied ? t('rooms.avOccupied') : undefined}
