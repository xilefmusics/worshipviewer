...
export function PlayerRoomThreePanelShell({ queue, player, details }: Props) {
  ...
  const panelLabel = (panel: Panel) => t(`playerRooms.panel.${panel}`)

  return (
    <div className="flex h-dvh min-h-0 flex-col overflow-hidden bg-[var(--color-background)]">
      <nav className="flex shrink-0 border-b border-[var(--color-border)] bg-[var(--color-surface)] lg:hidden" role="tablist" aria-label={t('playerRooms.panels')}>
        {panels.map((panel) => (
          <button
            key={panel}
            type="button"
