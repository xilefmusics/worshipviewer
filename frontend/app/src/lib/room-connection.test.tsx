...
describe('room connection', () => {
  ...
  it('sends the latest queued musical state after the room socket opens', () => {
    ...
    act(() => {
      result.current.sendMusicalState({ item_index: 1, started: true, language: null, transposition: null })
      result.current.sendMusicalState({ item_index: 2, started: true, language: null, transposition: null })
    ...
    expect(JSON.parse(socket.send.mock.calls[1][0] as string)).toMatchObject({
      ...
      musical_state: { item_index: 2, started: true, language: null, transposition: null },
    })
    unmount()
  })
})
