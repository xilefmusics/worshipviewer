...
describe('rooms', () => {
  it('uses the stable room name as the short display name', () => {
    expect(
      roomShortName({
        name: 'Room abc12345',
      } as Parameters<typeof roomShortName>[0]),
    ).toBe('Room abc12345')
  })
  ...
  it('stores returned room credentials only after successful creation', async () => {
    ...
    await createRoom({ team_id: 'team-1' })
  ...
  it('retains no room credential when creation fails', async () => {
    ...
    await expect(createRoom({ team_id: 'team-1' })).rejects.toThrow('forbidden')
  ...
  it('applies a contiguous room delta without replacing immutable content', () => {
    ...
    const result = applyRoomServerMessage(current, {
      ...
      musical_state: { item_index: 0, started: false, language: null, transposition: '2' },
      revision: 5,
    })
