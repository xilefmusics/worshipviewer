...
| [epic-e8.1-action-plan.md](./epic-e8.1-action-plan.md) | E8.1 player role variants (AV first) checklist |
...
| 2026-05-21 | **E6 (songs v1):** Song editor overflow **Import** + **Export** (ChordPro, Worship Pro, PDF); songs hub long-press **Export**; **`+`** chooser **New** \| **Import** batch via `POST /api/v1/songs`; shared `song-import-export.ts` + Vitest. Hub long-press **Export** for setlists and collections via `run-setlist-export.ts` / `run-collection-export.ts`. Single merged ChordPro bundle per setlist/collection remains deferred. | [epic-e6-action-plan.md](./epic-e6-action-plan.md), [song-editor.md](./song-editor.md). |
| 2026-05-23 | **Roadmap extension:** added **E8.1 Player role variants (AV mode first)** between E8 and E9; AV projection is modeled as a player variant (not a separate subsystem), with future slots for roles like click/pad. | [roadmap.md](./roadmap.md), [epic-e8.1-action-plan.md](./epic-e8.1-action-plan.md). |
| 2026-05-23 | **E8.1 grill decisions:** existing mode labeled **Normal**; primary tap opens player in global default mode; context menu adds explicit **Play in Normal mode** and **Play in AV mode**; no in-player mode toggle in E8.1; AV requires dual-window output with single-screen fallback warning; AV settings live in dedicated global `/settings` tab with player quick access. | [epic-e8.1-action-plan.md](./epic-e8.1-action-plan.md), [pages-and-flows.md](./pages-and-flows.md), [app-shell.md](./app-shell.md). |
| 2026-05-23 | **E8.1.x — Restore legacy presenter parity:** Pre-Chorus + Verse 5–9 keyboard jumps, distinct **r** / **Shift+R** for blank vs blackout, **o** opens projection window. | [legacy-vs-frontend2-analysis.md](./legacy-vs-frontend2-analysis.md). |
