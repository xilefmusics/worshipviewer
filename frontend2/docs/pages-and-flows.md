...
| `/collections` | Collections list | Default **card** view (A4 cover aspect). **Primary tap / Enter / Space** → **`/player`** (`type=collection`). **FAB +** → **`CreateCollectionDialog`** (`?new=1` latch); success → **`/collections/:id`**. **Edit** via long-press / context menu. Long-press **Export** → ChordPro / Worship Pro (`.zip` of `.cp` / `.wp`) or **PDF (print)** ([E6](./epic-e6-action-plan.md)). |
...
| `/setlists` | Setlists list | Default **list** view. **FAB +** opens **create setlist** when the team library is writable; after create, navigate to **`/setlists/:id`**. Long-press **Export** → ChordPro / Worship Pro (`.zip`) or **PDF (print)** ([E6](./epic-e6-action-plan.md)). |
